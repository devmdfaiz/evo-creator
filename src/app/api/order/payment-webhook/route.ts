...
import { Transaction, Wallet } from "@/lib/mongodb/models/wallet.model";
...
export async function POST(req: Request) {
  try {
    ...
    const cashfreeWebhookResponse = await req.json()
    ...
    // Function to update order and wallet
    ...
    if (cashfreeWebhookResponse.type === "PAYMENT_SUCCESS_WEBHOOK") {
      ...
      const { error_description, error_description_raw } =
        cashfreeWebhookResponse.data.error_details;
      await updateOrder({
        paymentMethod,
        paymentStatus,
        error: { errorDescription: error_description, errorDescriptionRaw: error_description_raw },
      });
    }
